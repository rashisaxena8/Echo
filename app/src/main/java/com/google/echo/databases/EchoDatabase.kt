package com.google.echo.databases
        ...
        try {
            ...
            if (cSor.moveToFirst()) {
                do {
                    ...
                    var _id = cSor.getInt(cSor.getColumnIndex(Staticated.COLUMN_ID))
                    var _artist = cSor.getString(cSor.getColumnIndex(Staticated.COLUMN_SONG_ARTIST))
                    var _title = cSor.getString(cSor.getColumnIndex(Staticated.COLUMN_SONG_TITLE))
                    var _songPath = cSor.getString(cSor.getColumnIndex(Staticated.COLUMN_SONG_PATH))
                    _songList.add(Songs(_id.toLong(), _title, _artist, _songPath, dateAdded = 0))
